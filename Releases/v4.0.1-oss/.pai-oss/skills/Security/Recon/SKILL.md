...
`~/.pai-oss/PAI/USER/SKILLCUSTOMIZATIONS/Recon/`
...
- `naabu` - Port scanning
- Note: Requires security MCP profile (`~/.pai-oss/MCPs/swap-mcp security`)
...
Reports saved to:
- **Work directory** (`~/.pai-oss/MEMORY/WORK/{current_work}/`) - For iterative artifacts during investigation
- **MEMORY/RESEARCH/** (`~/.pai-oss/MEMORY/RESEARCH/YYYY-MM/`) - For pentest engagements and formal assessments
...
~/.pai-oss/MCPs/swap-mcp security
...
Assistant: Activating recon skill with passive-recon workflow.
     ...
     Report: ~/.pai-oss/MEMORY/WORK/{current_work}/recon-example-com/
```
...
**Security Skills:**
- `~/.pai-oss/skills/Investigation/` - Entity and people reconnaissance (OSINT)
- `~/.pai-oss/skills/Security/WebAssessment/` - Web application testing
