...
TELOS is {PRINCIPAL.NAME}'s life framework stored in `~/.pai-oss/PAI/USER/TELOS/`. It contains:
...
!`FILE="$1"; CONTENT="$2"; DESCRIPTION="$3"; bun ~/.pai-oss/commands/update-telos.ts "$FILE" "$CONTENT" "$DESCRIPTION"`
...
!`head -50 ~/.pai-oss/PAI/USER/TELOS/updates.md`

## View Specific TELOS File
!`FILE="$1"; cat ~/.pai-oss/PAI/USER/TELOS/"$FILE"`
...
Example:
```bash
bun ~/.pai-oss/commands/update-telos.ts "BOOKS.md" "- *Project Hail Mary* by Andy Weir" "Added favorite book: Project Hail Mary"
```
...
The script is at: `~/.pai-oss/commands/update-telos.ts`

All backups are stored in: `~/.pai-oss/PAI/USER/TELOS/Backups/`

All changes are logged in: `~/.pai-oss/PAI/USER/TELOS/updates.md`
