 ...
 *
 * Configuration files:
 *   Base:  ~/.pai-oss/skills/Agents/Data/Traits.yaml
 *   User:  ~/.pai-oss/PAI/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml
 *
 * Usage:
...
const BASE_TRAITS_PATH = `${HOME}/.pai-oss/skills/Agents/Data/Traits.yaml`;
const USER_TRAITS_PATH = `${HOME}/.pai-oss/PAI/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml`;
const TEMPLATE_PATH = `${HOME}/.pai-oss/skills/Agents/Templates/DynamicAgent.hbs`;
const CUSTOM_AGENTS_DIR = `${HOME}/.pai-oss/custom-agents`;
...
/**
 * Save a composed agent to ~/.pai-oss/custom-agents/{slug}.md
 *
 * Produces a CLAUDE CODE COMPATIBLE agent file that can be:
 * 1. Copied to ~/.pai-oss/agents/ and used as a built-in agent
 ...
 *
 * Matches the structural format of built-in agents in ~/.pai-oss/agents/*.md:
...
\`\`\`bash
bun run ~/.pai-oss/skills/Agents/Tools/ComposeAgent.ts --load "${slug}"
\`\`\`

Or reconstruct from traits:

\`\`\`bash
bun run ~/.pai-oss/skills/Agents/Tools/ComposeAgent.ts --traits "${agent.traits.join(",")}"
\`\`\`
...
OPTIONS:
  ...
  -s, --save           Save composed agent to ~/.pai-oss/custom-agents/
...
CONFIGURATION:
  Base traits:    ~/.pai-oss/skills/Agents/Data/Traits.yaml
  User traits:    ~/.pai-oss/PAI/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml
  Custom agents:  ~/.pai-oss/custom-agents/
