...
`~/.pai-oss/PAI/USER/SKILLCUSTOMIZATIONS/OSINT/`
...
```
~/.pai-oss/MEMORY/WORK/$(jq -r '.work_dir' ~/.pai-oss/MEMORY/STATE/current-work.json)/YYYY-MM-DD-HHMMSS_osint-[target]/
```

**Archived reports:**
```
~/.pai-oss/History/research/YYYY-MM/[target]-osint/
```
