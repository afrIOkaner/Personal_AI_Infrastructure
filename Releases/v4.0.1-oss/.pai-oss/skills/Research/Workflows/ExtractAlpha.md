...
```bash
~/.pai-oss/MEMORY/WORK/{current_work}/
```

**To get the current work directory:**
1. Read `~/.pai-oss/MEMORY/STATE/current-work.json`
2. Extract the `work_dir` value
3. Use `~/.pai-oss/MEMORY/WORK/{work_dir}/` for temporary artifacts
...
```
~/.pai-oss/MEMORY/WORK/20260111-172408_extract-alpha-analysis/
├── raw-transcript.txt
...
```bash
~/.pai-oss/History/research/YYYY-MM-DD_description/
```
...
```
~/.pai-oss/History/research/2025-10-26_podcast-analysis/
...
1. **Check if hooks captured the output:**
   ...
   ls -lt ~/.pai-oss/History/research/ | head -5

   # Verify your research directory exists
   ls ~/.pai-oss/History/research/YYYY-MM-DD_description/
   ```

2. **If hooks did NOT capture automatically:**
   ```bash
   # Create directory structure manually
   mkdir -p ~/.pai-oss/History/research/YYYY-MM-DD_description/
...
3. **Confirm all files saved:**
   ```bash
   ls -lah ~/.pai-oss/History/research/YYYY-MM-DD_description/
...
WORK_DIR=$(jq -r '.work_dir' ~/.pai-oss/MEMORY/STATE/current-work.json)

# 2. Work in current work item directory
cd ~/.pai-oss/MEMORY/WORK/${WORK_DIR}/
...
mkdir -p ~/.pai-oss/History/research/$(date +%Y-%m-%d)_podcast-analysis/
...
ls -lah ~/.pai-oss/History/research/$(date +%Y-%m-%d)_podcast-analysis/
...
2. **Get current work directory** - Read `~/.pai-oss/MEMORY/STATE/current-work.json` for `work_dir`
3. **Use work item directory** - Work in `~/.pai-oss/MEMORY/WORK/{work_dir}/`
...
6. **Save to history** - Final outputs to `~/.pai-oss/History/research/YYYY-MM-DD_description/`
