...
`~/.pai-oss/PAI/USER/SKILLCUSTOMIZATIONS/Research/`
...
**Artifacts persist** at `~/.pai-oss/MEMORY/RESEARCH/{date}_{topic}/` — the vault survives across sessions.
...
**Working files (temporary work artifacts):** `~/.pai-oss/MEMORY/WORK/{current_work}/`
- Read `~/.pai-oss/MEMORY/STATE/current-work.json` to get the `work_dir` value
...
**History (permanent):** `~/.pai-oss/History/research/YYYY-MM/YYYY-MM-DD_[topic]/`
