...
`~/.pai-oss/PAI/USER/SKILLCUSTOMIZATIONS/Browser/`
...
**Headless by default.** All automation runs headless. When the user says "show me", open the URL in their preferred browser from `~/.pai-oss/PAI/USER/TECHSTACKPREFERENCES.md`:
...
**Agent definitions:** `~/.pai-oss/agents/BrowserAgent.md` and `~/.pai-oss/agents/UIReviewer.md`
