...
`~/.pai-oss/PAI/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/`
...
Read and analyze:
- ~/.pai-oss/MEMORY/STATE/current-work.json
...
Analyze:
- ~/.pai-oss/skills/ (installed skills)
- ~/.pai-oss/hooks/ (active hooks)
- ~/.pai-oss/settings.json (configuration)
...
Run: bun ~/.pai-oss/skills/Utilities/PAIUpgrade/Tools/Anthropic.ts
...
**User Customizations** (`~/.pai-oss/PAI/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/`):
