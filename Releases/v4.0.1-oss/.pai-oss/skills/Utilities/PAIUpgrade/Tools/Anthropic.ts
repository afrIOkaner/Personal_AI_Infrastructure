...
// Config
const HOME = homedir();
const SKILL_DIR = join(HOME, '.pai-oss', 'skills', 'PAIUpgrade');
...
function generateRecommendation(update: Update): string {
  ...
  if (titleLower.includes('command') || titleLower.includes('slash command')) {
    return `**PAI Impact:** HIGH - Command system update\n` +
      `**Why:** PAI uses slash commands extensively (~/.pai-oss/Commands/). Changes affect our command architecture and user workflows.\n` +
