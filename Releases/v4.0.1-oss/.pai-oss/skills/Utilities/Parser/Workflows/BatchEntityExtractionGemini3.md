...
const entityIndex = JSON.parse(
  fs.readFileSync('~/.pai-oss/skills/parser/entity-index.json', 'utf-8')
);
...
```typescript
import { scrapeAsMarkdown } from '~/.pai-oss/skills/mcp/Providers/brightdata/actors';
...
// Main processing
const entityIndexPath = '~/.pai-oss/skills/parser/entity-index.json';
...
const schema = JSON.parse(
  fs.readFileSync('~/.pai-oss/skills/parser/schema/content-schema.json', 'utf-8')
);
...
processedArticles.forEach((article: any, index: number) => {
  ...
  if (!valid) {
    ...
    const filename = `~/.pai-oss/skills/parser/output/${timestamp}_batch-${index + 1}.json`;
...
```bash
# Check entity counts
cat ~/.pai-oss/skills/parser/entity-index.json | jq '{
...
URLS=("$@")
SKILL_DIR="$HOME/.pai-oss/skills/parser"
...
const entityIndexPath = `${process.env.HOME}/.pai-oss/skills/parser/entity-index.json`;
...
processedArticles.forEach((article: any, index: number) => {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
  const filename = `${process.env.HOME}/.pai-oss/skills/parser/output/${timestamp}_batch-${index + 1}.json`;
