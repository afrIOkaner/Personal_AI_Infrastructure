...
```bash
~/.pai-oss/skills/[skill-name]/SKILL.md
```
...
```bash
cp -r ~/.pai-oss/skills/[skill-name]/ ~/.pai-oss/History/Backups/[skill-name]-backup-$(date +%Y%m%d)/
```

**Note:** Backups go to `~/.pai-oss/History/Backups/`, NEVER inside skill directories.
...
cd ~/.pai-oss/skills/[SkillName]/Workflows/
...
find ~/.pai-oss/skills/[SkillName]/ -type d -mindepth 2 -maxdepth 3
```
