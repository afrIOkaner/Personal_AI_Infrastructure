...
### Skill Directory
```bash
ls ~/.pai-oss/skills/ | grep -i [skillname]
```
...
```bash
ls -la ~/.pai-oss/skills/[SkillName]/
```
...
```bash
bun ~/.pai-oss/skills/[SkillName]/Tools/[ToolName].ts --help
```
...
```bash
grep -l "Intent-to-Flag" ~/.pai-oss/skills/[SkillName]/Workflows/*.md
```
...
**Reference:** `~/.pai-oss/PAI/CliFirstArchitecture.md`
