...
1. **Load Art preferences:**
   ```
   ~/.pai-oss/PAI/USER/SKILLCUSTOMIZATIONS/Art/PREFERENCES.md
   ```
...
3. **Use Theme Constants:**
   ```
   ~/.pai-oss/skills/Remotion/Tools/Theme.ts
   ```

4. **Reference images** (when visual style reference needed):
   ```
   ~/.pai-oss/skills/Media/Art/Examples/
   ```

## PAI Theme Quick Reference

```typescript
import { PAI_THEME } from '~/.pai-oss/skills/Remotion/Tools/Theme'
...
```typescript
import { PAI_THEME, titleScreenStyle, fadeInterpolation } from '~/.pai-oss/skills/Remotion/Tools/Theme'
