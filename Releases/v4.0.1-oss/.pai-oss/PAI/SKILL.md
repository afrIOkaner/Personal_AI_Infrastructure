...
- Create PRD at `~/.pai-oss/MEMORY/WORK/{session-slug}/PRD-{YYYYMMDD}-{slug}.md` via `generatePRDTemplate()`.
...
- **Mandatory output format.** Every response MUST use exactly one output format from PAI.md Execution Modes (ALGORITHM, NATIVE, ITERATION, or MINIMAL). No freeform output. No exceptions.
