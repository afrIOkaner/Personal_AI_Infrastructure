...
- Write PRD.md with Write tool — include YAML frontmatter (task, slug, effort, phase, progress, mode, started, updated) + sections (Context, Criteria, Decisions, Verification) per `~/.pai-oss/PAI/PRDFORMAT.md`
...
- **Mandatory output format** — Every response MUST use exactly one of the output formats defined in the Execution Modes section of PAI.md (ALGORITHM, NATIVE, ITERATION, or MINIMAL). No freeform output. No exceptions. If you completed algorithm work, wrap results in the ALGORITHM format. If iterating, use ITERATION. Choose the right format and use it.
...
4. `~/.pai-oss/MEMORY/STATE/work.json` has the registry of all sessions (populated by read-only PRDSync + PRDStateSync hooks)
...
**Full spec:** `~/.pai-oss/PAI/PRDFORMAT.md` (read during OBSERVE if needed for field details or continuation rules).
