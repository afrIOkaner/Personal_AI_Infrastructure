...
**Location:** `~/.pai-oss/PAI/Tools/Inference.ts`
...
bun ~/.pai-oss/PAI/Tools/Inference.ts --level fast "System prompt" "User prompt"

# Standard (Sonnet) - balanced reasoning, typical analysis
bun ~/.pai-oss/PAI/Tools/Inference.ts --level standard "System prompt" "User prompt"

# Smart (Opus) - deep reasoning, strategic decisions
bun ~/.pai-oss/PAI/Tools/Inference.ts --level smart "System prompt" "User prompt"

# With JSON output
bun ~/.pai-oss/PAI/Tools/Inference.ts --json --level fast "Return JSON" "Input"

# Custom timeout
bun ~/.pai-oss/PAI/Tools/Inference.ts --level standard --timeout 60000 "Prompt" "Input"
```
...
**Location:** `~/.pai-oss/PAI/Tools/RemoveBg.ts`
...
bun ~/.pai-oss/PAI/Tools/RemoveBg.ts /path/to/image.png

# Remove background and save to different path
bun ~/.pai-oss/PAI/Tools/RemoveBg.ts /path/to/input.png /path/to/output.png

# Process multiple images
bun ~/.pai-oss/PAI/Tools/RemoveBg.ts image1.png image2.png image3.png
```
...
**Location:** `~/.pai-oss/PAI/Tools/AddBg.ts`
...
bun ~/.pai-oss/PAI/Tools/AddBg.ts /path/to/transparent.png "#EAE9DF" /path/to/output.png

# Add brand background color
bun ~/.pai-oss/PAI/Tools/AddBg.ts /path/to/transparent.png --brand /path/to/output.png
```
...
**Location:** `~/.pai-oss/PAI/Tools/GetTranscript.ts`
...
bun ~/.pai-oss/PAI/Tools/GetTranscript.ts "https://www.youtube.com/watch?v=VIDEO_ID"

# Save transcript to file
bun ~/.pai-oss/PAI/Tools/GetTranscript.ts "https://www.youtube.com/watch?v=VIDEO_ID" --save /path/to/transcript.txt
```
...
**Technical Details:**
- Voice server must be running (`~/.pai-oss/skills/VoiceServer/`)
...
**Location:** `~/.pai-oss/PAI/Tools/extract-transcript.py`
...
**Location:** `~/.pai-oss/PAI/Tools/YouTubeApi.ts`
...
bun ~/.pai-oss/PAI/Tools/YouTubeApi.ts --channel-stats

# Get video statistics
bun ~/.pai-oss/PAI/Tools/YouTubeApi.ts --video-stats VIDEO_ID

# Get latest uploads
bun ~/.pai-oss/PAI/Tools/YouTubeApi.ts --latest-videos
```
...
1. **Add tool file:** Place `.ts` or `.py` file directly in `~/.pai-oss/PAI/Tools/`
...
2. **Document here:** Add section to this file with:
   - Tool location (e.g., `~/.pai-oss/PAI/Tools/ToolName.ts`)
   - Usage examples
