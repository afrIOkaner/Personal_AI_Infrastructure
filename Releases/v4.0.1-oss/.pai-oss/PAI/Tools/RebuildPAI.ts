 ...
 *
 * Usage: bun ~/.pai-oss/PAI/Tools/RebuildPAI.ts
 *
...
const PAI_DIR = join(HOME, ".pai-oss/PAI");
...
const SETTINGS_PATH = join(HOME, ".pai-oss/settings.json");
...
for (const file of components) {
  ...
  if (file === "00-frontmatter.md") {
    content = content.replace(
      "  Build:  bun ~/.pai-oss/PAI/Tools/RebuildPAI.ts",
      `  Build:  bun ~/.pai-oss/PAI/Tools/RebuildPAI.ts\n  Built:  ${timestamp}`
    );
  }
