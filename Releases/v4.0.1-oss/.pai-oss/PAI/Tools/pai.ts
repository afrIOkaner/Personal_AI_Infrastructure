 ...
 *   pai --local          Stay in current directory (don't cd to ~/.pai-oss)
...
const PAI_DIR = join(homedir(), ".pai-oss");
const MCP_DIR = join(PAI_DIR, "MCPs");
const ACTIVE_MCP = join(PAI_DIR, ".mcp.json");
const BANNER_SCRIPT = join(PAI_DIR, "PAI", "Tools", "Banner.ts");
...
async function cmdLaunch(options: { mcp?: string; resume?: boolean; skipPerms?: boolean; local?: boolean }) {
  // PAI.md is now static — no build step needed.
...
USAGE:
  ...
  k -l, --local            Stay in current directory (don't cd to ~/.pai-oss)
