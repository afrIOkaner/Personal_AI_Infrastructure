...
/**
 * BuildPAI.ts — Generate PAI.md from template + settings
 *
 * Reads PAI.md.template, resolves variables from settings.json
 * and PAI/Algorithm/LATEST, writes PAI.md.
 *
 * Called by:
 *   - PAI installer (first install)
 *   - SessionStart hook (keeps fresh automatically)
 *   - Manual: bun PAI/Tools/BuildPAI.ts
 */

import { readFileSync, writeFileSync, existsSync } from "fs";
import { join } from "path";

const PAI_DIR = join(process.env.HOME!, ".pai-oss");
const TEMPLATE_PATH = join(PAI_DIR, "PAI.md.template");
const OUTPUT_PATH = join(PAI_DIR, "PAI.md");
...
export function build(): { rebuilt: boolean; reason?: string } {
  if (!existsSync(TEMPLATE_PATH)) {
    return { rebuilt: false, reason: "No PAI.md.template found" };
  }
  ...
  if (existsSync(OUTPUT_PATH)) {
    ...
    if (existing === content) {
      return { rebuilt: false, reason: "PAI.md already current" };
    }
  }
...
if (import.meta.main) {
  ...
  if (result.rebuilt) {
    ...
    console.log("✅ Built PAI.md from template");
