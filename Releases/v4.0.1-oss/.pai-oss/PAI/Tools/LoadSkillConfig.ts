 ...
 *
 * Usage:
 *   import { loadSkillConfig } from '~/.pai-oss/PAI/Tools/LoadSkillConfig';
 *   const config = loadSkillConfig<MyConfigType>(__dirname, 'config.json');
 *
 * Or CLI:
 *   bun ~/.pai-oss/PAI/Tools/LoadSkillConfig.ts <skill-dir> <filename>
 */
...
// Constants
const HOME = homedir();
const CUSTOMIZATION_DIR = join(HOME, '.pai-oss', 'PAI', 'USER', 'SKILLCUSTOMIZATIONS');
...
Examples:
  bun LoadSkillConfig.ts ~/.pai-oss/skills/PAIUpgrade sources.json
