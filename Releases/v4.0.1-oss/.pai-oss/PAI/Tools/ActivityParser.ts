...
function categorizeFile(filePath: string): keyof ParsedActivity["categories"] | null {
  ...
  if (!filePath.includes("/.pai-oss/")) return null;
...
function getRelativePath(filePath: string): string {
  const claudeIndex = filePath.indexOf("/.pai-oss/");
  if (claudeIndex === -1) return filePath;
  return filePath.substring(claudeIndex + 9); // Skip "/.pai-oss/"
}
