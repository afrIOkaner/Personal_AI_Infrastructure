...
**Location:** `~/.pai-oss/projects/-Users-{username}--claude/`
...
ls ~/.pai-oss/MEMORY/WORK/ | tail -5
```

### Check ratings
```bash
tail ~/.pai-oss/MEMORY/LEARNING/SIGNALS/ratings.jsonl
```
...
ls -lt ~/.pai-oss/projects/-Users-{username}--claude/*.jsonl | head -5

# View last session events
tail ~/.pai-oss/projects/-Users-{username}--claude/$(ls -t ~/.pai-oss/projects/-Users-{username}--claude/*.jsonl | head -1) | jq .
```

### Check learnings
```bash
ls ~/.pai-oss/MEMORY/LEARNING/SYSTEM/
ls ~/.pai-oss/MEMORY/LEARNING/ALGORITHM/
ls ~/.pai-oss/MEMORY/LEARNING/SYNTHESIS/
```

### Check failures
```bash
# List recent failure captures
ls -lt ~/.pai-oss/MEMORY/LEARNING/FAILURES/$(date +%Y-%m)/ 2>/dev/null | head -10

# View a specific failure
cat ~/.pai-oss/MEMORY/LEARNING/FAILURES/2026-01/*/CONTEXT.md | head -100

# Migrate historical low ratings to FAILURES
bun run ~/.pai-oss/PAI/Tools/FailureCapture.ts --migrate
```

### Check multi-session progress
```bash
ls ~/.pai-oss/MEMORY/STATE/progress/
```

### Run harvesting tools
```bash
# Harvest learnings from recent sessions
bun run ~/.pai-oss/PAI/Tools/SessionHarvester.ts --recent 10

# Generate pattern synthesis
bun run ~/.pai-oss/PAI/Tools/LearningPatternSynthesis.ts --week
```
...
- Moved BACKUPS to `~/.pai-oss/BACKUPS/` (outside MEMORY)
- Renamed RAW-OUTPUTS to RAW
- All directories now ALL CAPS

**2026-01-05:** v1.0 - Unified Memory System migration
- Previous: `~/.pai-oss/history/`, `~/.pai-oss/context/`, `~/.pai-oss/progress/`
- Current: `~/.pai-oss/MEMORY/`
