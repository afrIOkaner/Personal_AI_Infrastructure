...
**Location:** `~/.pai-oss/hooks/`
**Configuration:** `~/.pai-oss/settings.json`
...
- **History Capture** - Automatic work/learning documentation to `~/.pai-oss/MEMORY/`
...
- Writes to `~/.pai-oss/MEMORY/SIGNALS/ratings.jsonl`
...
### Location
**File:** `~/.pai-oss/settings.json`
**Section:** `"hooks": { ... }`

### Environment Variables
Hooks have access to all environment variables from `~/.pai-oss/settings.json` `"env"` section:

```json
{
  "env": {
    "PAI_DIR": "$HOME/.pai-oss",
...
**Key Variables:**
- `PAI_DIR` - PAI installation directory (typically `~/.pai-oss`)
...
**Session Mapping:** `~/.pai-oss/MEMORY/STATE/agent-sessions.json`
```json
{
...
**Full Documentation:** See `~/.pai-oss/PAI/TERMINALTABS.md`
...
**Location:** `~/.pai-oss/hooks/my-custom-hook.ts`
...
```bash
chmod +x ~/.pai-oss/hooks/my-custom-hook.ts
```
...
```bash
# Test hook directly
echo '{"session_id":"test","transcript_path":"/tmp/test.jsonl","hook_event_name":"Stop"}' | bun ~/.pai-oss/hooks/my-custom-hook.ts
```
...
**Check:**
1. Is hook script executable? `chmod +x ~/.pai-oss/hooks/my-hook.ts`
2. Is path correct in settings.json? Use `${PAI_DIR}/hooks/...`
3. Is settings.json valid JSON? `jq . ~/.pai-oss/settings.json`
4. Did you restart Claude Code after editing settings.json?

**Debug:**
```bash
# Test hook directly
echo '{"session_id":"test","transcript_path":"/tmp/test.jsonl","hook_event_name":"Stop"}' | bun ~/.pai-oss/hooks/my-hook.ts

# Check hook logs (stderr output)
tail -f ~/.pai-oss/hooks/debug.log  # If you add logging
```
...
**Check:**
1. Does `~/.pai-oss/MEMORY/` directory exist?
2. Does current-work file exist? Check `~/.pai-oss/MEMORY/STATE/current-work.json`
3. Is hook actually running? Check `~/.pai-oss/MEMORY/RAW/` for events
4. File permissions? `ls -la ~/.pai-oss/MEMORY/WORK/`

**Debug:**
```bash
# Check current work
cat ~/.pai-oss/MEMORY/STATE/current-work.json

# Check recent work directories
ls -lt ~/.pai-oss/MEMORY/WORK/ | head -10
ls -lt ~/.pai-oss/MEMORY/LEARNING/$(date +%Y-%m)/ | head -10

# Check raw events
tail ~/.pai-oss/MEMORY/RAW/$(date +%Y-%m)/$(date +%Y-%m-%d)_all-events.jsonl
```
...
**Check:**
1. Is `~/.pai-oss/MEMORY/STATE/agent-sessions.json` writable?
...
cat ~/.pai-oss/MEMORY/STATE/agent-sessions.json | jq .

# Check subagent-stop debug log
tail -f ~/.pai-oss/hooks/subagent-stop-debug.log
```
...
grep '"type":"user"' ~/.pai-oss/projects/-Users-username--claude/*.jsonl | head -1 | jq '.type'
...
**Check:**
1. Does `~/.pai-oss/PAI/SKILL.md` exist?
...
bun ~/.pai-oss/hooks/LoadContext.hook.ts
...
- Permission denied → `chmod +x ~/.pai-oss/hooks/LoadContext.hook.ts`
...
- **Voice System:** `~/.pai-oss/VoiceServer/SKILL.md`
- **Agent System:** `~/.pai-oss/skills/Agents/SKILL.md`
- **History/Memory:** `~/.pai-oss/PAI/MEMORYSYSTEM.md`
...
KEY FILES:
~/.pai-oss/settings.json              Hook configuration
~/.pai-oss/hooks/                     Hook scripts (22 files)
~/.pai-oss/hooks/handlers/            Handler modules (6 files)
~/.pai-oss/hooks/lib/                 Shared libraries (13 files)
~/.pai-oss/hooks/lib/learning-utils.ts Learning categorization
~/.pai-oss/hooks/lib/time.ts          PST timestamp utilities
~/.pai-oss/hooks/lib/event-types.ts   Typed event definitions (22 interfaces)
~/.pai-oss/hooks/lib/event-emitter.ts appendEvent() → events.jsonl
~/.pai-oss/MEMORY/WORK/               Work tracking
~/.pai-oss/MEMORY/LEARNING/           Learning captures
~/.pai-oss/MEMORY/STATE/              Runtime state
~/.pai-oss/MEMORY/STATE/events.jsonl  Unified event log (append-only)

INFERENCE TOOL (for hooks needing AI):
Path: ~/.pai-oss/PAI/Tools/Inference.ts
...
| `build.*` | rebuild | BuildPAI (SessionStart handler) |
...
tail -f ~/.pai-oss/MEMORY/STATE/events.jsonl | jq

# Filter by type
tail -f ~/.pai-oss/MEMORY/STATE/events.jsonl | jq 'select(.type | startswith("algorithm."))'
