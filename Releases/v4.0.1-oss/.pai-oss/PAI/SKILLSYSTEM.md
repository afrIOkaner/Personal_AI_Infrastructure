...
- Reference `~/.pai-oss/PAI/USER/` for any personalization
...
- `~/.pai-oss/PAI/USER/CONTACTS.md` - Contact information
- `~/.pai-oss/PAI/USER/TECHSTACKPREFERENCES.md` - Tech preferences
```
...
`~/.pai-oss/PAI/USER/SKILLCUSTOMIZATIONS/{SkillName}/`
...
```
~/.pai-oss/PAI/USER/SKILLCUSTOMIZATIONS/
...
1. **Create directory**: `mkdir -p ~/.pai-oss/PAI/USER/SKILLCUSTOMIZATIONS/SkillName`
...
**See:** `~/.pai-oss/skills/Science/Protocol.md` for the full protocol interface
...
**Full documentation:** `~/.pai-oss/PAI/THENOTIFICATIONSYSTEM.md`
...
Read ~/.pai-oss/skills/Media/Art/Aesthetic.md
```
...
```
~/.pai-oss/skills/Createskill/Workflows/CanonicalizeSkill.md
```
...
5. Move backups to `~/.pai-oss/MEMORY/Backups/`
...
**Reference:** `~/.pai-oss/skills/_PERSONAL/_MYSKILL/SKILL.md`
...
**See:** `~/.pai-oss/PAI/CLIFIRSTARCHITECTURE.md` (Workflow-to-Tool Integration section)
...
**See:** `~/.pai-oss/PAI/CLIFIRSTARCHITECTURE.md` (Configuration Flags section) for full documentation
 ...
 *
 * Usage:
 *   bun ~/.pai-oss/skills/SkillName/Tools/ToolName.ts <command> [options]
 *
 * Commands:
