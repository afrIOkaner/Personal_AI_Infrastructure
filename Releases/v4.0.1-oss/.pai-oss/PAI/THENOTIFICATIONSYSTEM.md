...
**Voice phase announcements are inline curls in the Algorithm template** (defined in PAI.md), not hooks. Each Algorithm phase has a `curl -s -X POST http://localhost:8888/notify` call that gets spoken. The effort level determines which curls fire:
...
**Full voice registry:** `~/.pai-oss/skills/Agents/SKILL.md` (see Named Agents) and `~/.pai-oss/settings.json` (daidentity.voiceId)
...
Located in `~/.pai-oss/settings.json`:
...
The notification service is in `~/.pai-oss/hooks/lib/notifications.ts`:
