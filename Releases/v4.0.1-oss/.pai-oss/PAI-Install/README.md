...
| 5 | **PAI Repository** | Clones the PAI repo to `~/.pai-oss/` (or updates if already present) |
...
|------|----------|----------|
| `settings.json` | `~/.pai-oss/settings.json` | Merged config (template + user fields) |
| `.env` | `~/.config/PAI/.env` | `ELEVENLABS_API_KEY=...` |
| `LATEST` | `~/.pai-oss/PAI/Algorithm/LATEST` | Algorithm version (patched to current) |
| Shell alias | `~/.zshrc` | `alias pai='cd ~/.pai-oss && claude'` |
...
| Permission denied | Run `chmod -R 755 ~/.pai-oss` |
...
| Banner shows wrong algorithm version | Check `~/.pai-oss/PAI/Algorithm/LATEST` contains correct version |
...
- **Single-user** — Installs to `~/.pai-oss/` for the current user only
