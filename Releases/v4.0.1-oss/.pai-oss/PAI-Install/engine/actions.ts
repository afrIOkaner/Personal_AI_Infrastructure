...
/**
 * Search existing .pai-oss directories and config locations for a given env key.
...
function findExistingEnvKey(keyName: string): string {
  ...
  // Check ~/.pai-oss/.env
  searchPaths.push(join(home, ".pai-oss", ".env"));

  // Check any .pai-oss* directories (and legacy .claude*) in home (old versions, backups)
  try {
...
/**
 * Search existing .pai-oss directories for settings.json voice configuration.
...
function findExistingVoiceConfig(): { voiceId: string; aiName: string; source: string } | null {
  ...
  candidates.push(join(home, ".pai-oss", "settings.json"));

  // Scan all .pai-oss* directories (and legacy .claude*) (backups, renamed, etc.)
  try {
    const homeEntries = readdirSync(home);
    for (const entry of homeEntries) {
      if (entry.startsWith(".claude") && entry !== ".claude") {
...
export async function runRepository(
  ...
  const paiDir = state.detection?.paiDir || join(homedir(), ".pai-oss");
  ...
  if (state.installType === "upgrade") {
    ...
    if (cloneResult !== null) {
      ...
      if (initResult !== null) {
        ...
        await emit({
          event: "message",
          content: "Could not clone PAI repo automatically. You can clone it manually later: git clone https://github.com/danielmiessler/PAI.git ~/.pai-oss",
        });
      }
    }
  }
...
export async function runConfiguration(
  ...
  // Voice server reads ~/.env, hooks read ~/.pai-oss/.env
  if (existsSync(envPath)) {
    const symlinkPaths = [
      join(paiDir, ".env"),         // ~/.pai-oss/.env
...
export async function runVoiceSetup(
  ...
  if (hasElevenLabsKey) {
    ...
    // Ensure symlinks exist at both ~/.pai-oss/.env and ~/.env
