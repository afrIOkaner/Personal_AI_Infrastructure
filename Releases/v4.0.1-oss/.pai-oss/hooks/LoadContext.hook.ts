 ...
 *
 * PAI v4.0: Core context (identity, rules, format) is now in PAI.md and loaded
 ...
 *
 * DESIGN (v4.0):
 * PAI.md handles static identity/format (loaded natively by Claude Code).
...
async function checkActiveProgress(paiDir: string): Promise<string | null> {
  ...
  summary += '\n💡 To resume project: `bun run ~/.pai-oss/PAI/Tools/SessionProgress.ts resume <project>`\n';
  summary += '💡 To complete project: `bun run ~/.pai-oss/PAI/Tools/SessionProgress.ts complete <project>`\n';
...
async function main() {
  try {
    ...
    const isSubagent = claudeProjectDir.includes('/.pai-oss/Agents/') ||
...
---
Dynamic context loaded. Core identity, rules, and format are in PAI.md.
</system-reminder>`;
